...
options_driver_exp = ['5-10yr', '2-5yr', 'Above 10yr',
                      '1-2yr', 'Below 1yr', 'No Licence', 'unknown']
...
st.markdown("<h1 style='text-align: center;'>Accident Severity Prediction 🚧</h1>",
...
def main():
    with st.form('prediction_form'):
        ...
        day_of_week = st.selectbox(
            "Select Day of the Week: ", options=options_day)
        ...
        vehicles_involved = st.slider(
            "Pickup Hour: ", 1, 7, value=0, format="%d")
        ...
        accident_area = st.selectbox(
            "Select Accident Area: ", options=options_acc_area)
        driving_experience = st.selectbox(
            "Select Driving Experience: ", options=options_driver_exp)
    ...
    if submit:
        ...
        driving_experience = ordinal_encoder(
            driving_experience, options_driver_exp)
